...
<div class="card shadow-lg rounded card-bg">
    ...
    <div class="card-body">
        ...
        <a href="{{ url('/transactions/create') }}" class="btn btn-add-user mb-3" title="Add New Transaction">
                ...
                <tbody>
                    @forelse($transactions as $transaction)
                        <tr>
                            ...
                            <td>{{ $transaction->material->name ?? 'N/A' }}</td> <!-- Show material name -->
                            <td>{{ ucfirst($transaction->type) }}</td> <!-- Display 'add' or 'deduct' -->
                            <td>{{ $transaction->quantity }}</td>
                            <td>{{ $transaction->user->name ?? 'Unknown' }}</td> <!-- Show performed by user name -->
                            <td>
                                <!-- View Button -->
                                <a href="{{ url('/transactions/' . $transaction->id) }}" title="View Transaction">
                                ...
                                <!-- Edit Button -->
                                <a href="{{ url('/transactions/' . $transaction->id . '/edit') }}" title="Edit Transaction">
