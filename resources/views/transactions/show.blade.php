...
<div class="card shadow-lg rounded card-bg">
    ...
    <div class="card-body">
        ...
        <h5 class="card-title text-primary">Date: {{ \Carbon\Carbon::parse($transaction->created_at)->format('Y-m-d') }}</h5>
        
        <!-- Display Material -->
        <p class="card-text"><strong>Material:</strong> {{ $transaction->material->name ?? 'N/A' }}</p>

        <!-- Display Transaction Type -->
        <p class="card-text"><strong>Transaction Type:</strong> {{ ucfirst($transaction->type) }}</p>

        <!-- Display Quantity -->
        <p class="card-text"><strong>Quantity:</strong> {{ $transaction->quantity }}</p>

        <!-- Display Performed By User -->
        <p class="card-text"><strong>Performed By:</strong> {{ $transaction->user->name ?? 'Unknown' }}</p>

        <!-- Display Description if available -->
        <p class="card-text"><strong>Description:</strong> {{ $transaction->description ?? 'N/A' }}</p>

        <!-- Back to List Button -->
        <a href="{{ url('/transactions') }}" class="btn btn-secondary mt-3">
